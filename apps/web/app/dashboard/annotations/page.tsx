...
// Maps real DB annotation types to display labels + badge classes
...
export default async function AnnotationsPage() {
  ...
  const publishedCount = annotations.filter((a) => a.isPublished).length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {apiDown && (
        <div className="preview-banner">
          ...
          API not reachable — start the API server and run migrations to load peer annotations
        </div>
      )}
      ...
      {annotations.length > 0 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">
            <span className="font-semibold text-gray-700">{total}</span> annotations across the platform
          </p>
          <button className="btn-primary" aria-label="Submit a new clinical annotation">
      ...
      {annotations.length > 0 && (
        <div className="space-y-3">
          {annotations.map((ann) => {
            ...
            const endorsements = ann._count?.annotationEndorsements ?? 0;

            return (
              <article
                key={ann.id}
                className="card overflow-hidden"
                aria-label={`${typeConf.label}: ${ann.title}`}
              >
                <div className="p-5">
                  {/* Header */}
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      aria-label={`Endorse annotation: ${endorsements} endorsements`}
                    >
                  ...
                  {ann.device && (
                    <div className="mt-3 flex items-center gap-2 text-xs text-gray-500">
                      ...
                      <span className="font-medium text-gray-700">{ann.device.name}</span>
                  ...
                  {/* Title */}
                  <h2 className="mt-3 font-semibold text-gray-900 leading-snug">{ann.title}</h2>

                  {/* Body */}
                  <p className="mt-2 text-sm text-gray-600 leading-relaxed">{ann.body}</p>
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-gray-500">
                        ...
                        {ann.author.specialty && (
                          <>
                            ...
                            <span>{ann.author.specialty}</span>
                          </>
                        )}
                      </div>
                    )}

                    <time dateTime={ann.createdAt} className="text-xs text-gray-400">
                      Posted{" "}
        ...
        <div className="card flex flex-col items-center gap-4 px-6 py-10 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 max-w-sm">
              Verified clinicians can submit peer-reviewed observations. All submissions are reviewed before publication.
            </p>
          </div>
          <button className="btn-primary" aria-label="Submit a new clinical annotation">
      ...
      {/* Submit CTA (always shown when annotations exist) */}
      {annotations.length > 0 && (
        <div className="card flex flex-col items-center gap-4 px-6 py-10 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-50">
            <svg aria-hidden="true" className="h-6 w-6 text-brand-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
